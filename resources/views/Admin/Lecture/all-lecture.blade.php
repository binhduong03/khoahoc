...
@section('contents')
                                ...
                                <tbody>
                                    @foreach($lectures as $all)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex">
                                                    <a href="{{URL::to('Admin/edit-lecture/'. $all->lecture_id. '/'. $courseID->course_id)}}" class="btn btn-warning shadow btn-xs sharp me-1" title="Sửa"><i class="fas fa-pencil-alt"></i></a>
                                                    <a href="#" class="btn btn-primary shadow btn-xs sharp me-1" data-bs-toggle="modal" data-bs-target="#detail-lecture{{$all->lecture_id}}"
                                                    title="Xem"><i class="fas fa-eye"></i></a>
                                                    <a href="#" class="btn btn-danger shadow btn-xs sharp" data-bs-toggle="modal" data-bs-target="#deleteModal{{$all->lecture_id}}" title="Xóa"><i class="fa fa-trash"></i></a>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforeach
    ...
    @foreach($lectures as $all)
    <div class="modal fade" id="detail-lecture{{ $all->lecture_id }}" tabindex="-1" aria-labelledby="viewLectureLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="viewLectureLabel">Chi Tiết Bài Giảng</h5>
                ...
                <div class="modal-body">
                    <form>
                        ...
                        <div class="mb-3">
                            <label for="media-type" class="form-label">Loại tài liệu</label>
                            <input type="text" class="form-control" id="media-type" value="{{ $all->media_type }}" readonly>
                        </div>

                        @if ($all->media_type === 'VIDEO')
                            <div class="mb-3">
                                <label for="media-url" class="form-label">Video</label>
                                <video controls width="100%" height="400px">
                                    <source src="{{ asset('public/backend/images/lecture/' . $all->media_url) }}" type="video/mp4">
                                    Trình duyệt không hỗ trợ video.
                                </video>
                            </div>
                        @elseif ($all->media_type === 'PDF')
                            <div class="mb-3">
                                <label for="media-url" class="form-label">PDF</label>
                                <embed src="{{ asset('public/backend/images/lecture/' . $all->media_url) }}" type="application/pdf" width="100%" height="500px" />
                            </div>
                        @endif

                        <div class="mb-3">
                            <label for="order" class="form-label">Thứ tự hiển thị</label>
                            <input type="number" class="form-control" id="order" value="{{ $all->order }}" readonly>
                        </div>

                        <div class="mb-3">
                            <label for="status" class="form-label">Trạng thái</label>
                            <input type="text" class="form-control" id="status" value="{{ $all->status ? 'Hoạt động' : 'Không hoạt động' }}" readonly>
                        </div>
                    </form>
                </div>
    ...
    @foreach($lectures as $all)
    <div class="modal fade" id="deleteModal{{$all->lecture_id}}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" style="max-width: 400px;">
            <div class="modal-content text-center">
                <form action="{{route('admin.delete-lecture')}}" method="Post">
                    @csrf
                    <input type="hidden" name="lecture_id" value="{{$all->lecture_id}}">
